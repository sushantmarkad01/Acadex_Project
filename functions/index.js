...
// Endpoint for creating users (from before)
app.post('/createUser', async (req, res) => {
  try {
    ...
    // to ensure the caller is an authenticated institute admin.
    ...
    const userProfile = {
      uid: userRecord.uid, email, role, instituteId, firstName, lastName, ...extras,
    };
...
// ✅ NEW: Secure endpoint for marking attendance
app.post('/markAttendance', async (req, res) => {
    try {
        ...
        if (!classroomLocation) {
            return res.status(500).send({ error: 'Session was started without a location. Cannot verify attendance.' });
        }
